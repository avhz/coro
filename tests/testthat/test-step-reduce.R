...
test_that("reduce_steps() calls initial step for result completion", {
    ...
    steps <- compose(iter_map(`+`, 1), iter_map(`+`, 1))
    expect_error(
        reduce_steps(NULL, steps, builder),
        "called for init completion"
    )
})
...
test_that("collect() preserves type (#32)", {
    ...
    expect_equal(
        wait_for(async_collect(async_generator(
            function() for (i in 1:3) yield(i)
        )())),
        as.list(1:3)
    )
    ...
    expect_equal(
        wait_for(async_collect(async_generator(
            function() for (i in 1:3) yield(list(i))
        )())),
        lapply(1:3, list)
    )
    ...
    expect_equal(
        wait_for(async_collect(async_generator(
            function() for (i in 1:3) yield(mtcars[i, 1:3])
        )())),
        lapply(1:3, function(i) mtcars[i, 1:3])
    )
})
